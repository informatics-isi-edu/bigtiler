...
public class LociConversionImplementation implements ImageConversion {
    ...
    public static void main(final String[] args) {
        if (args.length < 2) {
            System.err.println("Usage: " + ConvertImage.class.getName()
     ...
     */
    public LociConversionImplementation(final String source,
    ...
    public void run() throws Exception {
        ...
        // find an image that meets our criteria for image size
        int series = -1;
        long currentPixelSize = 0;

        for (int i = 0; i < reader.getSeriesCount(); i++) {
            reader.setSeries(i);
            final long imagePixelSize = reader.getSizeX() * reader.getSizeY()
                    * reader.getSizeC();
            if (maximumImageSize == 0
                    || (imagePixelSize < maximumImageSize && imagePixelSize > minimumImageSize)) {
                if (imagePixelSize > currentPixelSize) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Image pixel size "
                                + imagePixelSize
                                + " in series "
                                + i
                                + " fits within bounds and is the largest series so far.");
                    }
                    series = i;
                    ...
                    LOG.debug("Image pixel size "
                            ...
                            + " in series "
                            + i
                            + " fits within bounds but is not the largest series.");
                }
            } else if (LOG.isDebugEnabled()) {
                LOG.debug("Image pixel size " + imagePixelSize + " in series "
                        + i + " doesn't fit within bounds.");
            }
        }

        if (series < 0) {
            throw new Exception(
                    "Cound not find an image series that fits within bounds.");
        }
        reader.setSeries(series);
        final LociImageSource source = new LociImageSource(reader, omexml, 0);
        final LociImageDestination destination = new LociImageDestination(
                destinationFile, (int) source.getWidth(),
                (int) source.getHeight(), omexml);
